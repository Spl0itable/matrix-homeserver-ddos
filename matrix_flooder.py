...
def get_random_proxy():
    try:
        return FreeProxy(anonym=True, rand=True).get()
...
def post_login():
    while True:
        ...
        proxies = {"http": get_random_proxy(), "https": get_random_proxy()} if use_proxies else None
        try:
...
def post_register():
    while True:
        ...
        try:
            ...
            response = requests.post(url, headers=headers, json=initial_data, proxies=proxies)
            if response.status_code == 200:
                print(colored(f"POST {response.status_code}: {url}", 'green'))
            elif response.status_code == 401:
                ...
                token_response = requests.post(url, headers=headers, json=token_data, proxies=proxies)
